...
def register_team_routes(app, db, User, Team, TeamMember, ChannelAccess, TeamInvitation, ConnectedPage,
    ...
    def team_dashboard():
        ...
        member_access_map = {}
        if primary_team:
            # Aggregate channel access for members
            members_data = []
            member_ids = [m.id for m in primary_team.members]
            if member_ids:
                access_rows = ChannelAccess.query.filter(ChannelAccess.team_member_id.in_(member_ids)).all()
                for access in access_rows:
                    member_access_map.setdefault(str(access.team_member_id), {})[str(access.channel_id)] = access.access_level
            ...
            for member in primary_team.members:
                access_map = member_access_map.get(str(member.id), {})
                members_data.append({
                    'id': member.id,
                    'user': member.user,
            ...
            return render_template('team/dashboard.html',
                                 ...
                                 is_owner=is_owner,
                                 is_admin=is_admin,
                                 member_access_map=member_access_map,
                                 now=datetime.utcnow())
    ...
    def team_invite(team_id=None):
        ...
        if request.method == 'POST':
            ...
            if not existing_user:
                flash(f"We couldn't find an account for {invited_email}. Ask them to sign up, then re-enter the email below.", 'warning')
                return redirect(url_for('team_invite', team_id=team.id))
            ...
            if team.owner_id == existing_user.id:
            ...
            from app import send_invitation_email
            email_sent = send_invitation_email(
                recipient_email=invited_email,
                recipient_name=invited_name,
                sender_name=inviter.username if inviter else 'Team Admin',
            ...
            if email_sent:
                ...
                flash(f'Invitation created for {invited_email}, but email could not be sent. Please share the invitation link manually.', 'warning')
    ...
    def team_member_channels(team_id, member_id):
        ...
        if request.method == 'POST':
            ...
            db.session.commit()
            success_message = f'Channel permissions updated for {member.user.username}'
